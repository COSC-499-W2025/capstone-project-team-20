...
class TestReportExporter:
    ...
    def test_init_sets_up_jinja_environment(self, exporter):
        ...
        assert exporter.latex_env is not None
        assert 'escape_latex' in exporter.env.filters
        assert 'escape_latex' in exporter.latex_env.filters
    ...
    def test_build_portfolio_context(self, exporter, mock_report_simple, mock_config_manager):
        context = exporter._build_portfolio_context(mock_report_simple, mock_config_manager)
        assert context["name"] == "John Doe"
        assert context["projects"] == mock_report_simple.projects

    def test_validate_portfolio_ready_missing_details(self, exporter):
        report = Mock()
        project = Mock()
        project.project_name = "Missing"
        project.portfolio_details = PortfolioDetails()
        report.projects = [project]

        with pytest.raises(ValueError, match="Missing"):
            exporter._validate_portfolio_ready(report)

    def test_get_environment_for_portfolio(self, exporter):
        env = exporter._get_environment("portfolio")
        assert env is exporter.latex_env

    def test_get_environment_for_resume(self, exporter):
        env = exporter._get_environment("jake")
        assert env is exporter.env
    ...
    def test_export_to_pdf_creates_tex_file(self, mock_file, mock_compile, mock_report_simple, mock_config_manager, tmp_path):
        ...
        exporter.latex_env = exporter.env
...
class TestTemplateRendering:
    ...
    def test_validate_portfolio_ready_success(self):
        proj = ReportProject(
            ...
            languages=["Python"],
            frameworks=[],
            portfolio_details=PortfolioDetails(project_name="good")
        )
        report = Report(None, "t", None, "resume_score", [proj], None)

        ReportExporter()._validate_portfolio_ready(report)

    def test_validate_portfolio_ready_missing(self):
        proj = ReportProject(
            project_name="bad",
            bullets=["x"],
            languages=["Python"],
