...
from utils.clone_repos import (ensure_gitignore,run_clone_workflow,clone_repos)
...
class TestRunCloneWorkflow:
    def test_successful_workflow(self):
        mock_csv_data = [{'repo_name': 'test-repo', 'repo_label': 'backend', 'repo_link': 'https://github.com/test/repo.git'}]
...
class TestCloneRepos:
    def test_successful_clone(self):
        repos = [{'repo_name': 'test-repo', 'repo_label': 'backend', 'repo_link': 'https://github.com/test/repo.git'}]
    ...
    def test_skip_existing_repo(self):
        repos = [{'repo_name': 'existing-repo', 'repo_label': 'backend', 'repo_link': 'https://github.com/test/repo.git'}]
        cloned_repos_dir = Path('cloned_repos')
        with patch('pathlib.Path.exists', return_value=True), \
             patch('subprocess.run') as mock_run, \
             patch('builtins.print'):
    ...
    def test_clone_subprocess_error(self):
        repos = [{'repo_name': 'bad-repo', 'repo_label': 'backend', 'repo_link': 'https://github.com/test/nonexistent.git'}]
        cloned_repos_dir = Path('cloned_repos')
