...
class ProjectAnalyticsSnapshot:
    ...
    All fields are derived from existing analyzers:
      ...
      - skills:       FolderSkillAnalyzer / SkillExtractor
      - authors:      GitRepoAnalyzer / ProjectManager
    """
    name: str
...
def assign_badges(snapshot: ProjectAnalyticsSnapshot) -> List[str]:
    ...
    # ---------- Skill-driven badges (via FolderSkillAnalyzer / SkillExtractor) ----------
...
def build_fun_facts(snapshot: ProjectAnalyticsSnapshot, badges: List[str]) -> List[str]:
    """
    Generate lightweight one-liner 'fun facts' for the analytics UI.
    """
    facts: List[str] = []
