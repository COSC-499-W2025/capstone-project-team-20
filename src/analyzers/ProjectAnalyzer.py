...
class ProjectAnalyzer:
    ...
    def clean_path(self, raw_input: str) -> Path:
        """
        Normalize a raw path string from user input, handling:
          - wrapping quotes
          - escaped characters on POSIX
          - user home expansion
        """
        stripped = raw_input.strip()

        # Only strip quotes if they wrap the entire path
        if (stripped.startswith('"') and stripped.endswith('"')) or (
            stripped.startswith("'") and stripped.endswith("'")
        ):
            stripped = stripped[1:-1]

        # On Windows, treat the input literally
        if os.name == "nt":
            return Path(os.path.expanduser(stripped))

        # On POSIX, unescape sequences like "\ " → " "
    ...
    def load_zip(self) -> bool:
        ...
        while not (os.path.exists(zip_path) and zipfile.is_zipfile(zip_path)):
            zip_path = self.clean_path(
                input("Invalid path or not a zipped file. Please try again: ")
            )
    ...
    def _aggregate_stats(
        ...
        authors_to_aggregate = (
            selected_authors if selected_authors is not None else author_stats.keys()
        )
    ...
    def _display_contribution_results(
        ...
        total_lines_edited_project = (
            total_stats.lines_added + total_stats.lines_deleted
        )
        total_lines_edited_selected = (
            selected_stats.lines_added + selected_stats.lines_deleted
        )

        if total_lines_edited_project > 0:
            project_share = (
                total_lines_edited_selected / total_lines_edited_project
            ) * 100
            print(
    ...
    def analyze_git_and_contributions(self) -> None:
        ...
        try:
            ...
            if usernames:
                ...
                self._display_contribution_results(
                    selected_stats, total_stats, usernames
                )
        finally:
    ...
    def change_selected_users(self) -> None:
        ...
        try:
            ...
            all_authors_list = self.contribution_analyzer.get_all_authors(
                str(repo_paths[0])
            )
    ...
    def analyze_skills(self) -> None:
        ...
        try:
            ...
            if not skills:
                ...
                print("\nDetected skills/tools:")
                for s in skills:
                    # s may be a dict or a dataclass-like object
                    if isinstance(s, dict):
                        name = s.get("skill") or s.get("name")
                    else:
                        name = getattr(s, "skill", None) or getattr(s, "name", None)
                    if name:
                        print(f"  - {name}")
            ...
            if project is not None:
                ...
                project.avg_functions_per_file = overall.get(
                ...
                # Also persist the raw skills list if desired
                project.skills_used = sorted(
                    {
                        (
                            s.get("skill")
                            if isinstance(s, dict)
                            else getattr(s, "skill", None)
                        )
                        for s in skills
                        if (
                            isinstance(s, dict)
    ...
    def analyze_badges(self) -> None:
        ...
        - Usernames from ConfigManager for author_count/collaboration_status

        Also persists badges back onto the Project record if it exists.
        """
        if not self.zip_path:
            print("No project loaded. Please load a zip file first.\n")
            return
        ...
        # 1) Metadata & category summary from the ZIP tree
        ...
        # 2) Language share + skills from extracted contents
        languages: Dict[str, float] = {}
        skills: Set[str] = set()

        temp_dir = extract_zip(str(path_obj))
        try:
            # Language share (percentage per language)
            languages = analyze_language_share(temp_dir) or {}

            # Skills via SkillAnalyzer
            skill_analyzer = SkillAnalyzer(Path(temp_dir))
            result = skill_analyzer.analyze()
        ...
        # 3) Author / collaboration info derived from selected usernames
        ...
        # 4) Persist badges back to the Project record if present
        ...
        if project is not None:
            ...
            # Optionally also keep languages/skills in sync
            project.languages = sorted(languages.keys())
            project.skills_used = sorted(skills)
            self.project_manager.set(project)

        # 5) Display project-level badges & fun facts
        if badge_ids:
            print("Badges:")
            for b in badge_ids:
                print(f"  - {b}")
        ...
        # 6) Portfolio-level badge summary across all projects
        all_projects = self.project_manager.get_all()
        totals = aggregate_badges(all_projects)

        print("\n=== Badge Summary Across All Projects ===")
        if totals:
            for badge_id, count in totals.items():
    ...
    def generate_resume_insights(self) -> None:
        ...
        the existing RepoFinder + ProjectMetadataExtractor.
        """
        ...
        while True:
            ...
            for proj in selected:
                ...
                if len(projects_list) == 1:
                    ...
                    if folder is None:
                        print(
                            f"[ERROR] Could not locate the folder for repo '{proj.name}' inside the ZIP."
                        )
                        print(
                            "Skipping this project to avoid incorrect global counts.\n"
                        )
                        continue
    ...
    def _find_folder_by_name(self, folder, target_name: str):
