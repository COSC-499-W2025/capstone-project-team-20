...
class ProjectAnalyzer:
    ...
    def analyze_git_and_contributions(self):
        ...
        try:
            ...
            # Step 1: Run a single, comprehensive analysis to get per-author stats.
            all_author_stats = self.contribution_analyzer.analyze(str(repo_paths[0]))
            all_authors_list = sorted(all_author_stats.keys())

            # Step 2: Use the results to get the active usernames.
            usernames = self._get_or_select_usernames(all_authors_list)

            if usernames:
                # Step 3: Aggregate stats from the data we already have, without re-analyzing.
    ...
    def run(self):
        while True:
            # Menu has been simplified and tidied.
            print("""
                ========================
                Project Analyzer
                ========================
