...
class SkillsChronology:
    ...
    def build_timeline(self, projects: Iterable[Dict]) -> List[SkillEvent]:
        events: List[SkillEvent] = []
        for p in projects or []:
            # find a usable date field
    ...
    def first_use_by_skill(self, projects: Iterable[Dict]) -> List[Tuple[str, date]]:
        """
        Return [(skill, first_seen_date)] sorted by first_seen_date then skill name.
        """
        first_seen: Dict[str, date] = {}
        for ev in self.build_timeline(projects):
